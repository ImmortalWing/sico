...
- Runtime v0 执行引擎已由 ADR-0002 选择 Wasmtime；Android Pulley 生产门槛仍待实测；
- WASI 0.3 是当前目标基线；兼容 adapter 和迁移策略仍开放；
- 是否需要后期直接原生机器码后端。
...
在创建正式编译器 Rust workspace 前，应完成可在仓库内执行的项目，并显式登记需要外部授权的实验：

1. ~~建立 AI-oriented 错误分类与固定覆盖~~（STEP-0014：12 类，真实 AI 频率未测量）；
2. ~~建立 AI 对现有程序进行概括、依赖追踪和影响分析的评测任务~~（STEP-0004/0007/0012）；
3. ~~编写 10 个覆盖 CLI、服务、数据处理和 UI 的代表性 Sico 程序草案~~（`examples/`）；
4. ~~为相同 P0 语义设计至少三套候选语法~~（A0/B/C 各 54 case）；
5. ~~建立三套语法的静态 token、生成/修复/理解协议与评分器~~（STEP-0012）；真实正确率需要模型、凭据和成本授权，按 `AGENT_GOAL.md` 外部递延，不产生虚构分数；
6. ~~定义最小值与类型语义~~（`SEMANTICS.md` 与 54 个 P0 cases）；
7. ~~定义形式化契约、效果、能力和语义索引草案~~（`SEMANTICS.md`、RFC-0002、STEP-0007）；
8. ~~定义诊断协议 v0 和错误编号分区~~（RFC-0001/STEP-0006）；
...
11. ~~根据结果确认 M1 的工程结构~~（[`M1 plan`](docs/plans/M1-compiler-frontend.md)）。
