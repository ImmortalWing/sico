...
Push-Location $root
try {
  & $CargoPath test --offline --locked -p sico-runtime -p sico-codegen-wasm -p sico-cli --all-targets --all-features --quiet
  if ($LASTEXITCODE -ne 0) { throw 'STEP-0036 tests failed' }
  & $CargoPath clippy --offline --locked -p sico-runtime -p sico-codegen-wasm -p sico-cli --all-targets --all-features -- -D warnings
  if ($LASTEXITCODE -ne 0) { throw 'STEP-0036 strict Clippy failed' }
  & $CargoPath build --offline --locked -p sico-cli --quiet
  ...
  foreach ($case in $runtimeCases) {
    $actual = (& $cli run --runtime $WasmtimePath (Join-Path $root $case.Source) | Out-String).Trim()
  ...
  $ErrorActionPreference = 'Continue'
  & $cli run --runtime (Join-Path $validation 'missing-wasmtime.exe') $answer 2>$errorFile | Out-Null
  ...
  Write-Output "STEP_0036_OK cli=build,run inputs=file,stdin artifact=component-wasm deterministic=sha256 runtime=wasmtime-46.0.1 corpus=int-bool-unit semantic_invalid=exit-1-no-artifact unsupported_runtime=exit-2 overwrite=refused entry=sync-scalar-main"
}
finally {
  Pop-Location
