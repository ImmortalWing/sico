...
foreach ($relative in @(
        ...
        'docs\steps\STEP-0207-m22-script-build-corpus.md'
    )) {
    if (-not (Test-Path -LiteralPath (Join-Path $root $relative))) {
        throw "missing STEP-0207 artifact: $relative"
    }
}

& (Join-Path $root 'tools\update-m22-corpus.ps1') -RepositoryRoot $root -Verify
if ($LASTEXITCODE -ne 0) { throw 'STEP-0207 corpus replay failed' }
...
Write-Output 'STEP_0207_OK corpus=215 script_accept=37 reproducible=37 script_refuse=178 lexical=116 script_abi=28 semantic=34 artifact_leaks=0'
