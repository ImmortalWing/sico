...
if ($bCases.Count -ne 58 -or $snapshot.Count -ne 58) {
...
if ($accept -ne 25 -or $semanticReject -ne 33) {
...
Write-Output "STEP_0017_OK b_cases=58 syntax_accept=58 designed_accept=$accept semantic_reject_not_executed=$semanticReject snapshots=$($snapshot.Count) lossless=pass"
