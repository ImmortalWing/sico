...
foreach ($needle in 'sico.ai-tool.request.v0','sico.ai-tool.response.v0','MAX_WORKSPACE_BYTES','MAX_CHANGED_BYTES','build_index','syntax_identity','original_sha256','expected_diagnostics','contiguous_edit','candidate_has_diagnostics','assert_eq!(rejected, 512)','assert_eq!(paths.len(), 54)','assert_eq!((clean, diagnosed), (25, 29))') {
...
Write-Output 'STEP_0068_OK tests=8 mutations=512 inspect=54 clean=25 diagnosed=29 fixes=12 ai_tasks=96 model_runs=0 credentials=0 cost_authorization=absent external_side_effects=0 next=STEP-0069'
