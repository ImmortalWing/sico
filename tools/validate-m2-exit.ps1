...
if ($m3Plan -notmatch '(?m)^> - status: ready after STEP-0029 GO\r?$') { throw 'M3 plan is missing after M2 GO' }
if (-not $audit.Contains('GO: M2 complete; M3 entry gate satisfied; next STEP-0030.')) { throw 'M2 audit has no exact GO conclusion' }
if ($status -notmatch '(?m)^> - phase: M3 ' -or $status -notmatch '(?m)^> - next step: STEP-0030\r?$') {
