...
> Status: in progress / NO-GO through STEP-0298 local evidence (STEP-0297 and STEP-0298 independent CI pending). S1/S2 declared subsets complete; S3/S4/S5 partial; S6/S7 not entered. ADR-0017 Option A and RFC-0047 are accepted. STEP-0292 independently verified W1 on e8495f9, STEP-0293 verified 22-C S1 on repaired cb371c5, STEP-0294 verified 22-C S2 on 5a6d0bb, STEP-0295 verified 22-C S3 on f7288b7, and STEP-0296 verified 22-C S4 on 904b86a, all on the isolated codex/m22-w1-ci branch. STEP-0297 moves the set_nearest_match typed frontier to EXPRESSION; STEP-0298 advances the formatter canary to 24/30 with match_arm_levels / CALL-SHAPE and R3 consecutive stall count 0. origin/dev has a conflicting parallel W1/W2 history and remains unmerged.
...
| S4 semantics + lowering (L2) | partial | formatter prefix byte-exact through `set_nearest_match` (24 of 30 functions; nested whiles via the STEP-0261 frame stack, while-less statement functions via the STEP-0262 last-resort routing); typed refusal-first discipline with re-pinned canary each step | the 6 remaining formatter functions (`match_arm_levels` … `main`), then the remaining selfhost sources lower byte-exactly |
...
1. **W1 exit and W2 baseline** — STEP-0292 obtained independent CI success
   ...
   returning match arms, advancing the canary to 24/30 with
   `match_arm_levels` / `CALL-SHAPE` next. R3 consecutive-stall count remains zero. Independent CI for
   ...
   independent CI run 36209299222 remains pending and STEP-0298 has not yet
   been submitted. The parallel `origin/dev` history is not adjudicated by this
   branch's CI.
