...
### 3.1 Slice status (measured 2026-09-23, through STEP-0261)
...
| S4 semantics + lowering (L2) | partial | formatter prefix byte-exact through `source_has_lex_error` (17 of 30 functions; nested whiles land via the STEP-0261 packed frame stack); typed refusal-first discipline with re-pinned canary each step | the 13 remaining formatter functions (`call_left` … `main`), then the remaining selfhost sources lower byte-exactly |
