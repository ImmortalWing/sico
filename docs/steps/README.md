...
| [STEP-0296](./STEP-0296-m22-w2-whileless-if-match-length.md) | local implementation complete / S4 independent CI pending（卡 22-C S4） | M22 compiler self-host — W2 S3/S4 | 无 while 的 if/match 路由与条件中 `sico.text.length(Text)` 字节差分通过；参数数目/类型 typed 拒绝并可恢复；canary 23/30，`set_nearest_match` 前沿 STATEMENT → CALL-SHAPE，R3 连续停滞数 0；M22 仍 NO-GO |
