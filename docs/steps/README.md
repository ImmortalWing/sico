...
| [STEP-0294](./STEP-0294-m22-w2-map-get-match-subject.md) | implementation complete locally / S2 independent CI pending（卡 22-C S2） | M22 compiler self-host — W2 S3/S4 | `map.get[Text,U64]` match 主语字节差分通过；多余参数/类型与错误 Map/key 类型 typed 拒绝、拒绝后恢复；Map 参数类型 JSON 括号缺口修复；formatter 22/30，`nearest_match` 前沿 CALL-TARGET → STATEMENT，R3 连续停滞数 0；0261/0262/0245 本机全绿，待本片独立 CI |
