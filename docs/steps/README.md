...
| [STEP-0297](./STEP-0297-m22-w2-nested-slice-subject.md) | complete / slice 5 GO on isolated branch（卡 22-C 切片 5） | M22 compiler self-host — W2 S3/S4 | 嵌套 `bytes.slice` match 主语字节差分通过；多余实参与错误类型 typed 拒绝并可恢复；canary 23/30，`set_nearest_match` 前沿 CALL-SHAPE → EXPRESSION（`map.put` return），R3 连续停滞数 0；`ffcdd3f` 独立 CI 运行 36209299222 成功，M22 仍 NO-GO |
| [STEP-0298](./STEP-0298-m22-w2-map-put-returning-match.md) | complete / slice 6 GO on isolated branch（卡 22-C 切片 6） | M22 compiler self-host — W2 S3/S4 | `map.put` return 与双臂 return match 的字节差分通过；多余实参与错误类型 typed 拒绝并可恢复；canary 24/30，`match_arm_levels` / CALL-SHAPE，R3 连续停滞数 0；`5ba175f` 独立 CI 运行 36210200271 成功，M22 仍 NO-GO |
| [STEP-0299](./STEP-0299-m22-w2-map-get-nested-key.md) | local implementation complete / independent CI pending（卡 22-C 切片 7） | M22 compiler self-host — W2 S3/S4 | 嵌套 `u64.to_text` 的 `map.get` key 字节差分通过；多余实参 typed 拒绝并可恢复；canary 编译计数 27/30，f27 前缀额外字节差分已绿；`opener_close` / GWSKIP，R3 连续停滞数 0；M22 仍 NO-GO |
| [STEP-0300](./STEP-0300-m22-w2-while-inside-if.md) | local implementation complete / independent CI pending（卡 22-C 切片 8） | M22 compiler self-host — W2 S3/S4 | if 的 then 臂内 while 帧保存父区域与深度，f28 前缀 16 块与 Rust IR 字节一致；canary 编译计数 28/30，`normalize_source` / GWPACK-OTHER，R3 连续停滞数 0；M22 仍 NO-GO |
