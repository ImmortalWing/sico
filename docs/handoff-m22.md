...
STEP-0294 是 22-C S2：`sico.map.get[Text,U64]` 的受限 match 主语与 Rust oracle 字节一致；错误 Map/key 类型、参数数和泛型数 typed 拒绝。formatter canary 仍 22/30，`nearest_match` 前沿 CALL-TARGET → STATEMENT，整份 exit 122，R3 连续停滞数仍为 0。本机 0261/0262/0245 全绿，**本片独立 CI 待跑**，因此 S2 尚未裁为 GO。
...
1. 将 STEP-0294 的确切实现提交送独立 CI；成功后裁决 22-C S2。再按 [STEP-0291](./steps/STEP-0291-m22-nearest-match-design.md) 开始 S3：match 臂内嵌 if/return 的控制流降低。
