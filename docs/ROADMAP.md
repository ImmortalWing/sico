...
> - updated: 2026-09-18 (remote/local M22 evidence merged through STEP-0221; support claims corrected to executable slice status)
...
状态：`in-progress：S1 formatter 已由 STEP-0217/0218 在冻结 215-source 语料关闭；S2 checker 仍 NO-GO；S3 lexer 已关闭、parser 为 accepted declaration metadata partial；S4 typed IR 由 STEP-0193–0212 扩至递归表达式、typed calls 与定宽运算但仍 partial；S5 只有 bounded codegen seam，S6 A=B=C 未进入。STEP-0214–0221 同时冻结 corpus/build oracle 并链接 modular frontend；自举架构门禁由 ADR-0015 / STEP-0198 关闭；下一门为 checked 算术（match 块）；后续 STEP 不预留`
