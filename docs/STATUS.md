...
> - updated: 2026-09-21 (STEP-0249 repairs the self-host 256-local build failure, lands PowerShell 5.1-runnable validators, re-executes the canonical-LF corpus freeze and re-pins the typed canary frontier at `E-SH-IR-CALL-TYPE`)
...
> - phase status: M11 complete (GO 2026-09-02); M12 complete (GO 2026-09-05, gates 10/10, Windows x64 + Linux x64 native runtime evidence); M22 compiler self-host track **S1/S2 complete；S3/S4/S5 partial；S6 尚未进入**。STEP-0249 修复 `control_while_function_ir` 256-local 越限（`while_bytes_at_rhs_packed` 抽取，busiest 238 locals + headroom 回归钉），self-host 组件恢复可构建，验证器与语料工具在本机 PowerShell 5.1 真实可跑，215/215 语料冻结真实执行，byte-exact IR 覆盖保持 formatter 前七函数。完整源码 canary、Component codegen 与 bootstrap 仍开放。
> - current step: STEP-0249 关闭自举树的构建失谐并如实重钉前沿；下一个 typed 拒绝区域为 while 体 if 条件操作数机械（不接受 `I64.literal(n)` 固定宽度字面量）与 `scalar_type_kind` 的 Bytes 参数缺口，M22 保持 NO-GO。
...
> - last completed active step: STEP-0249 (local-limit repair + frontier re-pin; M22 remains NO-GO)；此前 STEP-0248 (typed user-call guard-chain + Text return parity through `word_kind`)。
> - next step: 收敛 while 体 if 条件的固定宽度字面量操作数与 Bytes 参数支持，使 `scan_space` 通过并继续推进 formatter canary；完整 formatter/selfhost bundle 绿后才进入 RFC-0011 general Component codegen 与 ADR-0015 bootstrap harness。
