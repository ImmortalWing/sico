...
> - updated: 2026-09-18 (STEP-0208 constant call-argument IR)
...
> - phase status: M11 complete (GO 2026-09-02); M12 complete (GO 2026-09-05, gates 10/10, Windows x64 + Linux x64 native runtime evidence); M22 compiler self-host track **S1–S5 complete, S6 in progress** (STEP-0193–0208: lowering handoff → exact IR signatures → recursive expression tree → scalar/constant/multi-parameter IR → typed user calls → constant call arguments)
> - current step: owner 指令「完成自举，编写原生 UI 库，长期实现 GUI 文件格式转换器」——主线仍按门禁先关闭 M22 自举；STEP-0208 让调用实参支持单 token 常量（裸整数/Bool/字符串）：常量指令按源序先于 call 发射、result 自 caller 参数数起连续编号、原子 range 为源字节跨度、逐位 kind 检查，与 Rust lower_core byte-identical；累计 17 个正例均真实 build+runner + serde + verifier + Rust lower_core byte differential。下一步 fixed-width ops 与 fixed-width/嵌套调用实参；之后 codegen、自举闭环、原生 UI 与 GUI 转换器。
...
> - last completed active step: STEP-0208 (M22 constant call-argument IR：单 token 常量实参、源序常量指令、连续 result 编号、原子 range，与 Rust lower_core byte-identical；2/2 real-runner suite)；此前 STEP-0207 (cross-function call IR)；STEP-0206 (recursive user-call IR)。
> - next step: M22 S6 续行——扩展 typed IR：fixed-width operations、fixed-width/嵌套调用实参，逐片保持 Rust lower_core canonical JSON byte-identical；随后通用语句块、codegen（RFC-0011）与 ADR-0015 bootstrap harness；自举 GO 后再进入原生 UI/GUI 转换器。
