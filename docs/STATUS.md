...
> - updated: 2026-09-23 (STEP-0260 M22 line_tokens general-while 收敛：formatter 前十六函数至 `line_tokens` 与 Rust canonical IR byte-exact，general-while 机补齐通用调用条件/固定运算 set RHS/break/continue/List 类型面，完整源码 canary 推进至 nested while SKIP。此前 STEP-0259 M18 审计增补登记)
...
> - phase status: M11 complete (GO 2026-09-02); M12 complete (GO 2026-09-05, gates 10/10, Windows x64 + Linux x64 native runtime evidence); M22 compiler self-host track **S1/S2 complete；S3/S4/S5 partial；S6 尚未进入**。STEP-0253 以 parent-linked append-only frame + deferred empty-else block 对齐 nested/sequential if-without-else，并修复单指令计数与 return-call SSA 推进；formatter 前十三函数（至 `punctuation_kind`）与 Rust canonical IR byte-exact；STEP-0256 以 `list_get_match_ir`/`nested_intrinsic_return_ir` 落地 item 的 `sico.list.get` result-match 与 append_pair 嵌套 intrinsic return（`parameters_ir` 支持 `List[<scalar>]`），formatter 前十五函数（至 `append_pair`）byte-exact；STEP-0260 在 general-while 机内落地 `line_tokens`（32 分支 if/else 链 + 双 `sico.bytes.slice` match + 嵌套 `utf8_decode` 实参 + list-return 调用）：通用调用条件经 `while_call_rhs_packed` 实参打包、slice 主语逗号扫描修复、match 绑定改注册 `#match<binding>` 经 `local_binding_index` 回退解析、`List[<scalar>]` 类型 JSON 全链路透传（`type_json` 逐字嵌入）、固定运算（bit_and/or/xor/shl/shr）set RHS、`break`/`continue`（单 while，deferred end-if 组合），并回收 25 个一次性 dummy let 以贴近 `MAX_LOCALS_PER_FUNCTION=256`；formatter 前十六函数（至 `line_tokens`，52 block 含双 match 区）byte-exact，`selfhost_parser` 99 例差分与拒绝身份断言全绿；完整源码 canary 推进至 `format_code` 的 nested while（`SKIP:GENERAL-WHILE-NESTED`）。Component codegen 与 bootstrap 仍开放。
> - current step: STEP-0260 关闭 `line_tokens` 差分（general-while 机：通用调用条件、slice 逗号扫描、`#match<binding>` 绑定、List 类型面、固定运算 set RHS、break/continue），formatter 前十六函数 byte-exact；下一个 typed 边界为 `format_code` 的 nested while（`SKIP:GENERAL-WHILE-NESTED`），M22 保持 NO-GO。
...
> - last completed active step: STEP-0260 (line_tokens general-while convergence: 16-function formatter prefix byte-exact, break/continue + fixed-op set RHS, List type surface; M22 remains NO-GO)；此前 STEP-0256 (list.get result-match + nested intrinsic returns)、STEP-0253 (general-while no-else frames)。
> - next step: 为 self-host general lowering 接入 nested while（`format_code` 起的十一个函数至 `main`，当前 `SKIP:GENERAL-WHILE-NESTED`），继续沿 M22 计划 §3.2 执行队列收敛 formatter 尾部；完整 formatter/selfhost bundle 绿后才进入 RFC-0011 general Component codegen 与 ADR-0015 bootstrap harness。
