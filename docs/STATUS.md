...
> - updated: 2026-09-16 (M22 S6 续：STEP-0195 参数 arity 精确化 fn:add/2)；此前 2026-09-14 (M21 §3.1/§3.2 + M22 entry prereqs landed; M22 entry satisfied)
...
> - next step: M22 S6 继续——完整 lowering 到 typed IR（表达式树、块、参数类型结构化）+ Rust IR verifier 差分；自举架构 ADR（入口条件 5）仍为 S6 闭环前置；v1 batch 3 语言摩擦候选（中缀比较/逻辑运算符、表达式位置条件、裸字面量 AI 噪音复称）已记录于 M22 计划 §8，需各自 RFC；M17 剩余 §3.3/§3.4 合同草案；M18 block-game pilot 等 M17 整体 GO；平台 parity/外部试点/live-model 重测均 owner 门控
