...
> - updated: 2026-09-17 (STEP-0195 M22 S6 exact IR signatures；owner 指令「继续完成sico，M22-M25」驱动 M22 续行 + M23–M25 规划登记)
...
> - phase status: M11 complete (GO 2026-09-02); M12 complete (GO 2026-09-05, gates 10/10, Windows x64 + Linux x64 native runtime evidence); M22 compiler self-host track **S1–S5 complete, S6 in progress** (STEP-0193–0195: lowering handoff → exact IR signatures)
> - current step: owner 指令「继续完成sico，M22-M25」（2026-09-17）——主线为 M22 S6 续行（STEP-0195 关闭 STEP-0194 遗留：token 流携带标点 `(` `)` `,` `:` `[` `]`、数字入标识符分类、深度状态机计数 arity（空表 0、非空 逗号+1）、`end function` 终结符上下文、`;` 分隔签名；双函数语料 `fn:main/0;fn:add/2;` 经真实 `sico build` + 真实 runner byte-exact 1/1；STEP-0194 登记的「逗号差一」理论据实修正为「标点从未进入 token 流」）；下一步 S6 续：语句级 shape → 表达式 → typed IR（Rust IR verifier 为 oracle）→ codegen（RFC-0011 byte-exact）→ 自举闭环（自举架构 ADR 为前置）；并行登记 M23–M25 规划（owner 指令授权，见 ROADMAP）
...
> - last completed active step: STEP-0195 (M22 S6 续：exact IR signatures——`selfhost/parser.sico` token 流新增单字符标点与数字标识符分类、count_params 改括号深度状态机（空表 0/非空 逗号+1）、summarize 增加 `end function` 终结符上下文与 `;` 分隔；双函数语料 `fn:main/0;fn:add/2;` 经真实 build+runner 1/1 byte-exact；同时登记 fresh-host 环境重建（GNU 工具链重装、msys2 binutils PATH、wasmtime 46.0.1 沿用）与两处预存在发现：run-ci workspace 测试先于 runner 构建的顺序依赖（packages_resolve 兜底路径）、console-control 测试失败，STEP-0196 修正根因为缺 Python 依赖）；此前 STEP-0194 (M22 S6 续：parser 提取函数名+参数计数 fn:main/0——IR 签名 shape；带参数函数的计数差一登记为 parser 精化项，STEP-0195 据实修正根因为标点缺失）；此前 STEP-0193 (fn:main; lowering handoff) (M22 S6 开启：parser 提取函数名 fn:main;——第一个 lowering 级 shape，parser→lowering handoff 在 Sico 打通；提交 c3d7dc5 保全 M21 GO + M22 S1-S5)；此前 STEP-0192 (M22 S5 parser 消费 token 流) (M22 S5：Sico parser 消费 STEP-0191 token 流计数声明关键字——function=2 end=1 record=0 byte-exact，lexer→parser seam 在 Sico 打通；与 Rust outline 的 declarations-vs-occurrences 差异如实登记)；此前 STEP-0191 (token 7/7 + 根因=应用层大写分类 bug) (M22 S4 解决：token 缺陷**根因=应用层 is_ident_byte 大写字母分类 bug**[b<97 提前拦截 A-Z，后续大写检查成死码]，**非 lowering bug**——撤回 STEP-0185–0190 的编译器结论；修复后 token 7/7 字节级正确，spans 可信，S5 解锁)；此前 STEP-0182–0190 为 S3/S4 迭代（lexer 分类走通、token 发射调试、循环 cell 缺陷排查链）；此前 STEP-0177–0181 (M21 GO 6/6 + M22 S1/S1.5/S2：formatter 字节级差分绿、checker 骨架+差分 harness、ends_with 原语)；此前 STEP-0174/0175/0176 (M21 §3.1/§3.2：stdlib batch 2 + for-loops/错误传播 + List[I64]/[U64] 标量家族)；此前 STEP-0173 (M22 自举轨道规划)；此前 STEP-0172 (sico-app run --script 缝闭合)、STEP-0171 (M21 规划)、STEP-0170 (M20 出口审计 NO-GO 诚实登记 + §13 逐项表)、STEP-0169 (语言 v1 batch 1：RFC-0044 中缀 == lowering e2e byte-exact)、STEP-0168 (M19 执行与出口审计：run-ci.ps1 8/8 GREEN、可复现构建演练、registry 演练)
> - next step: M22 S6 续行——语句级 shape（while/if/return/let/set 语句 inventory）→ 表达式解析 → typed IR 发射（Rust IR verifier 为差分 oracle）→ codegen（RFC-0011 byte-exact）→ 自举闭环（自举架构 ADR 为 S6 闭环前置）；M23–M25 规划已按 owner 指令「继续完成sico，M22-M25」（2026-09-17）登记于 ROADMAP（contract-only，不预留实现 STEP）；v1 batch 3 语言摩擦候选（中缀比较/逻辑运算符、表达式位置条件、裸字面量 AI 噪音复称）为 M23 首批 RFC 候选；M17 剩余 gate 2/4 与 M18 block-game pilot 为 M24 主体；平台 parity/外部试点/live-model 重测均 owner 门控
