...
> - current step: STEP-0107 bounded channels, streams and backpressure (next; cancellation tree and race/select landed)
> - current support step: 无（M13 STEP-0119 已完成）
> - last completed active step: STEP-0106 (downward cancellation tree, scheduler-level race/select with canonical winner and loser abandon, timer readiness, typed deadlock detection, 1,024-chain cancellation evidence)
...
> - next step: implement bounded task-aware channels/streams with backpressure over the STEP-0105/0106 scheduler core (STEP-0107), then validator and full regression
...
下一项执行 M11 STEP-0107：在 STEP-0105/0106 scheduler core 与 cancellation tree 之上实现 bounded task-aware channels/streams 与 backpressure（1,024 items + 显式字节预算、deterministic close/cancel 清理），随后 STEP-0108 watch/REPL/DAP task 集成。实现期间不得改变 M10 冻结的 bounded events、redaction 与 exact DAP claims；race/select 源码拼写仍待独立 RFC；TLS HTTP 顺延 M12。公网 rollout、真实 pilot/model 与移动平台仍服从各自外部 gate。
