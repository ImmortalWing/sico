...
> - updated: 2026-09-23 (STEP-0261 M22 嵌套 while 栈帧：general-while 机单槽 while 帧改打包帧栈，首个嵌套 while 函数 `source_has_lex_error` 与 Rust canonical IR byte-exact，formatter 前十七函数收敛；canary 推进至 `call_left`。证据机 2026-09-23 更换为 GNU + 仓库 msys2-binutils 流。此前 STEP-0260 M22 line_tokens general-while 收敛)
...
> - current step: STEP-0261 关闭嵌套 while 差分（帧栈 + list.length/split_lines 参数面 + 嵌套调用实参），formatter 前十七函数 byte-exact；下一个 typed 边界为 `call_left`（guard-chain 2 参/set 臂/裸 cell 条件）与 `format_code`（gw body-if 用户调用条件），M22 保持 NO-GO。
