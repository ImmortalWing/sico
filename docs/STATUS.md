...
> - updated: 2026-09-24 (STEP-0277：W1 合并债计划登记 + 实测侦察——legacy 三件套（lexer/tokens/declaration_parser）selfhost 零 import 只被三单测 + 校验器 0215/0216/0219 + 0245 suite 引用（tokens.sico 裸 `<` 缺陷分支属实）；E7001 已结构性、E7002 关键词指纹待改写；sha 门实测已存在（selfhost_checker.rs:159-168，STEP-0246 批），验证关闭；缩进代理实位 compiler_parser.sico:489/650，0-indent 语料先行；实现分片 A→B→C→D 随后落地、CI 裁决。此前 STEP-0276：W1 前置 gate——中断会话在途 WIP 吸收（`Map[K,V]` 参数面 +228 行 + `count_params` 括号深度 + `dump_nm_region_ir` 探针）+ STEP-0272 selfhost 微分回归修复（guest `scalar_ir` 补 `records` 表发射，6 新函数，record 名字典序 = `sico.list.sort` byte-order，空表省略保持冻结字节）；本机 CLI 微分：formatter 前缀 14/16 byte-exact（nearest_match −393B 回归实证修复；close_code/direct_close 的 GWPACK-OTHER 经 HEAD 对照归因为既有非 gated 前沿）、driver --emit-ir 16 代表例 16/16、拒绝面 3 例精确；runner 测试仓本机不可重建，全量断言以 CI 裁决；W1 合并债前置已清，下一步 = W1 本体。此前 STEP-0275：for 迭代 over `List[record]` 落地 + 嵌套深度核对——v0 深度 2 文法层不可构造（E2031 三态保险），无预算债；records v0 全封（STEP-0271..0275），下一步 = W1 合并债。此前 STEP-0274：D5 `List[record]` ADR-0016 列式单态 empty/length/get/append 落地，`record-list-of-records.sico` run 解锁 `record-list-of-records-ok`，既有 15 语料零回归；v0 收窄 = 元素记录字段限 I64/U64、越界 error-tag 未定义；下一步 = for 迭代 over `List[record]` + W1 合并债。此前 STEP-0270：owner 指令「接受」登记 RFC-0047 翻 accepted，并完成接受后第一个实现前 STEP = EC-4 双普查冻结——selfhost 16,931 行 list 仪式行 360（2.13%）、应用 e2e map-as-struct 去重字面量键 14 个，冻结证据 [`m22-ec4-census-2026-09-24.json`](./reports/m22-ec4-census-2026-09-24.json)；下一步 = EC-1..EC-3 语料冻结。此前 STEP-0264 M22–M26 路线重划片按 owner 指令登记：M22 执行队列外加 R 阶段收敛门（R0 架构决策 SOA 冻结 ADR vs `List[record]` RFC 二选一、R1 canary 覆盖率与消耗燃料预算曲线仪器化、R3 预登记止损——连续 4 个实现 STEP canary 零增长且拒绝前沿不动即宣告 S6 在当前表面不收敛、触发 M23 Route B）；M23 实现门改双出口（Route A：S7 GO 后 / Route B：止损宣告后立即）；M24 即刻可开工盘点（入口门不依赖 M22/M23）；M25 入口要求 M22 行在双实现登记中显式。纯规划登记，不改任何能力合同/gate 数值/证据等级；详见 [`M22–M26 route replan v1`](./plans/M22-M26-route-replan-v1.md)。此前 STEP-0262 formatter 尾部三函数 byte-exact，前二十二函数收敛）
...
> - phase status: M11 complete (GO 2026-09-02); M12 complete (GO 2026-09-05, gates 10/10, Windows x64 + Linux x64 native runtime evidence); M22 compiler self-host track **S1/S2 complete；S3/S4/S5 partial；S6 尚未进入**。STEP-0261 把 general-while 机的单槽 while 帧（`ghdr_id`/`gbody_id`/`gafter_id` 全局）改为打包帧栈（`gwh_frames` + `gwh_len` 弹帧计数），while-open 在 entry/body/after 区域均合法（嵌套与顺序 while），if 臂内开 while 保持 typed 拒绝 `SKIP:GENERAL-WHILE-IF-REGION`；`end while` 按帧弹栈回填 after 块（emission 前沿分配，与 Rust `sort_by_key` 块序一致）、按 `brk_base` 作用域 seal 本帧 break、登记 branch 终结子表 `gwt_entries` 并按栈深回退 body/after 区域；配套落地 `sico.list.length` 参数/主语 cell、`sico.text.split_lines` 非字面量实参、用户调用嵌套调用实参（递归 `while_call_rhs_packed` 打包，内层 call 携带精确源 span，call 尾随检查放宽至行尾/`)`/`,`）；首个嵌套 while 函数 `source_has_lex_error`（外层行循环 + 内层 token 循环 + 循环体内 return）byte-exact，formatter 前十七函数（至 `source_has_lex_error`，10 block 双层 while 机）与 Rust canonical IR byte-exact（`tools/fixtures/step-0261/shle_expected.json` 117,109 字节钉住），`selfhost_compiler` 19/19、`selfhost_parser` 99 例差分与拒绝身份断言全绿，`general_while_function_ir` 240 locals 恰在界内（busiest+16≤256）；全量 canary fail-closed 推进至 `call_left`（`ERR:E-SH-IR-EXPRESSION`：guard-chain 机仅收 1 参且无 set 臂/裸 cell 条件/嵌套链；`format_code` 另需 general-while 机 body-if 的用户调用条件）。Component codegen 与 bootstrap 仍开放。
> - current step: STEP-0262 关闭 formatter 尾部三函数（gw 机 while-less 最后回退路由 + bare-cell 条件 + entry 控制流 + 按开序编号的 deferred 空 else + `sico.text.concat` 含嵌套实参的 intrinsic RHS），formatter 前二十二函数（至 `repeat_indent`）byte-exact；下一个 typed 边界为 `nearest_match` 的 `Map[Text,U64]` 参数面（`ERR:E-SH-IR-UNRESOLVED`），M22 保持 NO-GO。
> - current support step: STEP-0259 按 owner 确认完成 M18 审计增补登记（纯文档、不改任何 gate）：回答「M18 修改了是否需重做」——不需要；STEP-0164 已交付的四项 pilot 证据与 STEP-0165 出口审计保持有效（2026-09-22 重定仅触及从未完成的组合项 4 目标），登记「M24 转换器试点过 §3 gate 5 后执行 M18 出口审计增补：按 M18 计划 §4 验收路径审计、组合项 4 由 NO-GO 翻 GO、重跑 §8 gate 7 回归、更新 ROADMAP/STATUS M18 状态行、独立 STEP 留档；外部 pilot NO-GO 为 owner 门控外部门、增补不触碰」于 M18 计划 §7（登记文本同步 ROADMAP M18 状态行）。此前 STEP-0258 删除 block-game 试点、重定带 GUI 格式转换器（M24 计划更名重写、STEP-0257 superseded、§8.6.1 界面库工作流保留、image-vision@1 无 codec I/O 如实登记）；STEP-0257 曾登记方块游戏 GUI 试点（同日被 0258 取代）。
...
> - last completed active step: STEP-0277 (W1 合并债计划登记 + 实测侦察：legacy 三件套零 import/只被单测与历史校验器引用、E7002 关键词指纹待改写、sha 门已存在（STEP-0246 批）验证关闭、缩进代理 0-indent 语料先行；实现分片 A→B→C→D 随后)；此前 STEP-0276 (W1 前置 gate: 在途 WIP 吸收 [`Map[K,V]` 参数面 + `count_params` 括号深度 + 探针] + selfhost `records` 表发射修复——formatter 前缀微分 14/16 byte-exact、driver --emit-ir 16/16、拒绝面精确；close_code/direct_close GWPACK-OTHER 经 HEAD 对照为既有非 gated 前沿)、STEP-0275 (RFC-0047 D5 tail: for-iteration over `List[record]` + nesting depth audit — depth 2 unconstructible at grammar level, no budget debt; corpus run `record-list-of-records-ok` with for-accumulation gate)、STEP-0274 (RFC-0047 D5: `List[record]` column-layout monomorphs empty/length/get/append; corpus run unlocked; 15-corpus zero regression; records v0 closed)、STEP-0262 (format_code region: 22-function prefix byte-exact; M22 remains NO-GO)、STEP-0261 (nested-while stack frames)、STEP-0260 (line_tokens convergence)、STEP-0256 (list.get result-match)。
> - next step: M22 R 阶段已收口前半（R0 决策 + R1 仪器化实测完成，STEP-0264 重划片）：R0 = owner 已接受 Option A、RFC-0047 accepted、EC-4 双普查冻结（STEP-0270），R1 = canary 30 函数覆盖 22 + 燃料预算曲线无悬崖（STEP-0266）；**records v0 已全封（STEP-0271..0275）：EC-1/EC-2 语料 + E2021..E2024 拒绝面 + script-v0 ABI 扁平化 + D5 列式 `List[record]` 单态与 for 迭代（gate 语料含 for 累加校验，run 解锁）+ EC-5 矩阵行登记，嵌套深度 2 文法层不可构造无预算债；在途 WIP 已吸收（STEP-0276，`records` 表发射回归同片修复，formatter 前缀微分 14/16 byte-exact）；下一步 = M22 主线进 W1 合并债本体 + S3/S4 records re-baseline；records 剩余小债 = record-nested 多行字面量 + EC-3 canonical IR JSON 快照**；R 阶段后半（执行队列按 canary 增长恢复推进，`Map[Text,U64]` 参数面已随 WIP 吸收进 guest——nearest_match 前缀微分 byte-exact（STEP-0276 实测）；下一技术前沿 = `close_code`/`direct_close`/`opener_close`（首个 while-in-if，触及 SKIP:GENERAL-WHILE-IF-REGION，guest 现诚实拒绝 GWPACK-OTHER）→ `normalize_source`/`main`）与语料冻结不冲突，连续 4 个实现 STEP canary 零增长且拒绝前沿不动即触发 R3 止损评审；R0 决策文档已起草并按 owner 标准（2026-09-24「我只看最终效果和稳定性、长期可维护性」，STEP-0268 留痕）修订推荐 = [`ADR-0017`](../adr/ADR-0017-selfhost-data-model-architecture-v0.md)（STEP-0267/0268，owner 2026-09-24 已按 Option A 方向接受；**2026-09-24 owner 按推荐接受 Option A 方向并指令「开始」，R0 产出 = records RFC 草案 [`RFC-0047`](../rfc/RFC-0047-record-types-v0.md)（STEP-0269，status: draft，接受前不得实现）；同日 owner 指令「接受」后 RFC-0047 翻 accepted（STEP-0270），接受后第一个实现前 STEP = EC-4 双普查已冻结：selfhost 16,931 行 list 仪式行 360（2.13%）、应用 e2e map-as-struct 去重字面量键仅 14 个（冻结证据 [`m22-ec4-census-2026-09-24.json`](./reports/m22-ec4-census-2026-09-24.json)）**；推荐依据 = Option B 的"永久"是假永久（M14/M23 需求压力不消失，B = SOA 维护 + 迟到重写；A = 只重写一次 + 终局单一数据模型）；records 最小封闭集走 RFC-0033 全门，SOA 不变式在 re-baseline 前持续强制，W1 合并债照还）+ R0 输入 = [`M22 restart assessment v1`](./plans/M22-restart-assessment-v1.md)（STEP-0265）+ STEP-0266 R1 实测数字；在途 WIP（`parser.sico` Map 参数面 +228 行 + `dump_nm_region_ir` 探针）**已吸收为 STEP-0276（连同 `records` 表发射回归修复），W1 合并债前置已清**；W1 已登记为 STEP-0277（四债实测侦察：legacy 三件套零 import、E7002 指纹待改写、sha 门已存在待关闭、缩进代理语料先行），实现分片 A→B→C→D 随后落地；M23 开工盘点（四探针测量）与 M24 开工盘点可与 R 阶段并行启动
> - roadmap decision: owner approved the application-layer sequence on 2026-09-04 — M14 application-ready language, M15 Web/UI, M16 Native Automation Host, M17 vision/model packages, M18 representative AI applications and external pilots; M19 production engineering, M20 cross-platform/language v1/completion audit, M21 DX/stdlib batch 2/ecosystem activation and M22 compiler self-host followed by owner session directives (2026-09-13/14); on 2026-09-17 the owner directive 「继续完成sico，M22-M25」 confirmed continuing M22 and registering M23–M25 (language v1 batch 3, vision closure + block-game pilot, release v1.0 completion audit); on 2026-09-22 the owner directives 「方块游戏pilot改为GUI和原生界面库（风格类似winui3）」+「原生界面库是sico语言原生界面库」(STEP-0257, superseded same day) and 「方块游戏pilot目标删除，改为带GUI界面的格式转换器，第一步可以先实现图片格式互相转换，例如jpg互转png」(STEP-0258) deleted the block-game automation pilot, re-designated the M18 portfolio item 4 as a first-party GUI format converter (Sico-language native UI library, WinUI 3-like Fluent style; first increment JPEG↔PNG), kept the 俄罗斯方块 case as the M14 solver acceptance oracle only, and registered the Sico-language native UI library as an M24 prerequisite workstream (plan §8.6.1; gate texts, evidence classes and capability contracts unchanged, no implementation claimed); M13 quality-budget gap (STEP-0128) is a recorded product fact for M17/M18 planning; on 2026-09-24 the owner directive 「确认，要使用sico原生UI库」 confirmed and registered M26 — PDF document processing: structural reading, merge and rotate delivered as a versioned package (RFC-first, fail-closed corpora, no core language or authority change), plus a GUI PDF tool authored over the Sico-language native UI library (M24 plan §8.6.1), placed after M25 so the v1.0 verdict is not delayed （STEP-0263; no implementation STEP numbers reserved); same-day owner directive 「如果原生UI库不完善那就完善M24」(2026-09-24) registered as the M26 GUI scheduling rule: kickoff inventory gaps in the native UI library are closed under the M24 §8.6.1 workstream before the M26 GUI gate starts, package slices proceed independently, no web-UI fallback; same-day owner directive 「重新规划路线，审视M22-M26，确保能落实到工程实现」(2026-09-24) registered as STEP-0264 — the M22–M26 route replan: M22 gains an R-phase convergence gate (R0 SOA-freeze-ADR vs `List[record]`-RFC decision, R1 canary-coverage + consumed-fuel budget instrumentation, R3 pre-registered stop-loss of 4 consecutive STEPs with zero canary growth and unmoved refusal frontier), M23's implementation gate becomes dual-exit (Route A after M22 S7 GO / Route B immediately after a stop-loss declaration), M24 kickoff is unblocked in parallel, and M25's entry gate requires an explicit M22 verdict in the dual-implementation register; planning and gates only, no capability contract or evidence class changed, no implementation STEP numbers reserved (see [`M22–M26 route replan v1`](./plans/M22-M26-route-replan-v1.md))
