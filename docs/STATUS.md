...
> - updated: 2026-09-13 (M19/M20 execution + audits, M21 planned)
...
> - last completed active step: STEP-0172 (sico-app run --script 缝闭合：runtime 按 manifest script 身份分派 wasi:cli command 执行 + WASI CLI world 授能 + 退出码透传，e2e 4/4 经真实 wasmtime；release 残留登记：IExpress 跳过/SDK zip+install-windows.ps1 为受支持路径、打包 -AllowDirty 残留、暂态 E0308；answer.sico 登记为 scalar-profile build fixture 类)；此前 STEP-0171 (M21 规划)、STEP-0170 (M20 出口审计 NO-GO 诚实登记 + §13 逐项表)、STEP-0169 (语言 v1 batch 1：RFC-0044 中缀 == lowering e2e byte-exact)、STEP-0168 (M19 执行与出口审计：run-ci.ps1 8/8 GREEN、可复现构建演练、registry 演练)
> - last completed support step: STEP-0123 (M13 closure audit: a/b/c GO, d blocked-external-evidence)
> - next step: M21 开工（stdlib 第二批 RFC + byte/text 访问——最老实测缺口；v1 batch 2 RFC：裸字面量决策/for-loops/错误传播）；M17 剩余 §3.3/§3.4 合同草案；M18 block-game pilot 等 M17 整体 GO；平台 parity/外部试点/live-model 重测均 owner 门控
