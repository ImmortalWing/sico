...
> - updated: 2026-09-18 (STEP-0210 fixed-width literal call-argument IR)
...
> - phase status: M11 complete (GO 2026-09-02); M12 complete (GO 2026-09-05, gates 10/10, Windows x64 + Linux x64 native runtime evidence); M22 compiler self-host track **S1–S5 complete, S6 in progress** (STEP-0193–0210: lowering handoff → exact IR signatures → recursive expression tree → scalar/constant/multi-parameter IR → typed user calls → constant/fixed-width-literal call arguments → fixed-width operations)
> - current step: owner 指令「完成自举，编写原生 UI 库，长期实现 GUI 文件格式转换器」——主线仍按门禁先关闭 M22 自举；STEP-0210 让用户调用实参接受定宽字面量（I64.literal/U64.literal，含负数）：kind 对 callee 形参逐位检查、SSA 编号沿 STEP-0208 规则、指令 data 为无引号数值、range 仅覆盖数字位（负数含符号），与 Rust lower_core byte-identical；累计 25 个正例均真实 build+runner + serde + verifier + Rust lower_core byte differential。下一步嵌套调用实参（递归下降）与定宽运算的字面量实参；之后 checked 算术、通用语句块、codegen、自举闭环、原生 UI 与 GUI 转换器。
...
> - last completed active step: STEP-0210 (M22 fixed-width literal call-argument IR：字面量实参 kind/范围校验、数字位 range、负号 data，与 Rust lower_core byte-identical；2/2 real-runner suite)；此前 STEP-0209 (fixed-width operation IR)；STEP-0208 (constant call-argument IR)。
> - next step: M22 S6 续行——扩展 typed IR：嵌套调用实参（递归下降）、定宽运算的字面量实参，逐片保持 Rust lower_core canonical JSON byte-identical；随后 checked 算术（match 块）、通用语句块、codegen（RFC-0011）与 ADR-0015 bootstrap harness；自举 GO 后再进入原生 UI/GUI 转换器。
