...
> - updated: 2026-09-23 (STEP-0262 M22 formatter 尾部：`call_left`/`format_code`/`repeat_indent` 三函数 byte-exact，formatter 前二十二函数收敛；canary 推进至 `nearest_match` 的 Map[Text,U64] 参数面。此前 STEP-0261 嵌套 while 栈帧)
...
> - last completed active step: STEP-0262 (format_code region: call_left/format_code/repeat_indent byte-exact via gw last-resort routing, concat intrinsic RHS, bare-cell conditions, open-order deferred else blocks; 22-function prefix; M22 remains NO-GO)；此前 STEP-0261 (nested-while stack frames)、STEP-0260 (line_tokens convergence)、STEP-0256 (list.get result-match)。
> - next step: 收敛 formatter 尾部剩余 7 个函数：`Map[Text,U64]` 参数面（`nearest_match`/`set_nearest_match`/`match_arm_levels`，`scalar_type_kind` 不认 Map 需专扩）、`close_code`/`direct_close`/`opener_close`（首个 while-in-if 函数，触及 `SKIP:GENERAL-WHILE-IF-REGION` 边界）、`normalize_source`、`main`；完整 formatter/selfhost bundle 绿后才进入 RFC-0011 general Component codegen 与 ADR-0015 bootstrap harness。
