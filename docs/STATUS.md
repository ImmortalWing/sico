...
> - updated: 2026-09-05
> - phase: M12 complete (full GO per STEP-0127); M13 closed on dev with the authoritative live-model run (STEP-0128: budgets NOT met, measured); M7 public deployment and M6/mobile remain deferred
...
> - current step: 无内部主线（M12 完整 GO；M13 STEP-0119–0123 + 0128 完成——live-model 实测 0.9095 未达 ADR-0011 floor，§13 (d)=实测 NO-GO；下一主线为 M14 应用就绪语言基线，待 owner 启动决策后预留 STEP）
...
> - next support step: 无（M13 已收口：live-model 实测完成，预算未达标为既录事实；质量追赶属后续 owner 决策——guide/格式化器/模型快照/superseding ADR）
...
[`STEP-0128`](./steps/STEP-0128-live-model-evaluation.md)（dev 分支）已按 M13 §5 插入规则完成权威 live-model 评测：DeepSeek `deepseek-chat`，96×30=2880 次真实调用，成本 $0.78（封顶 $10），实测 **0.9095**（2245/2880；generation 0.40/0.50/0.30、repair 0.914/0.833/0.994、understanding 1.00/1.00/0.999）。对照 ADR-0011：overall 与 generation/repair floor **FAIL**，§13 目标预算未达——(d) 项由 blocked-external-evidence 变为**实测 NO-GO**；预算按 ADR-0011 §4 不做调整。报告：[`docs/reports/ai-eval-live-model-v1.md`](./reports/ai-eval-live-model-v1.md)。
