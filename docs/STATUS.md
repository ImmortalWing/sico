...
> - updated: 2026-09-17 (STEP-0207 typed cross-function call IR)
...
> - phase status: M11 complete (GO 2026-09-02); M12 complete (GO 2026-09-05, gates 10/10, Windows x64 + Linux x64 native runtime evidence); M22 compiler self-host track **S1–S5 complete, S6 in progress** (STEP-0193–0207: lowering handoff → exact IR signatures → recursive expression tree → scalar/constant/multi-parameter IR → typed user calls)
> - current step: owner 指令「完成自举，编写原生 UI 库，长期实现 GUI 文件格式转换器」（2026-09-17）——主线仍按门禁先关闭 M22 自举；STEP-0207 把 lowering 从单函数推广到多函数模块：函数 id 按声明序 1 基分配、调用目标全模块名字解析（含前向引用）、caller/callee 参数表分离的逐位 type/arity 检查、重复定义与未定义目标 typed 拒绝，且与 Rust lower_core byte-identical；累计 13 个正例均真实 build+runner + serde + verifier + Rust lower_core byte differential。下一步常量/嵌套调用实参与 fixed-width ops；之后 codegen、自举闭环、原生 UI 与 GUI 转换器。
...
> - last completed active step: STEP-0207 (M22 cross-function call IR：多函数模块、声明序函数 id、全模块名字解析、caller/callee 分离 type/arity、四类 typed 拒绝与 Rust lower_core byte-identical；2/2 real-runner suite)；此前 STEP-0206 (recursive user-call IR)；STEP-0205 (multi-scalar parameter IR)。
> - next step: M22 S6 续行——扩展 typed IR：常量/嵌套调用实参、fixed-width operations，逐片保持 Rust lower_core canonical JSON byte-identical；随后通用语句块、codegen（RFC-0011）与 ADR-0015 bootstrap harness；自举 GO 后再进入原生 UI/GUI 转换器。
