...
> - updated: 2026-09-18 (STEP-0211 nested call-argument IR)
...
> - phase status: M11 complete (GO 2026-09-02); M12 complete (GO 2026-09-05, gates 10/10, Windows x64 + Linux x64 native runtime evidence); M22 compiler self-host track **S1–S5 complete, S6 in progress** (STEP-0193–0211: lowering handoff → exact IR signatures → recursive expression tree → scalar/constant/multi-parameter IR → typed user calls → constant/fixed-width-literal/nested-call arguments → fixed-width operations)
> - current step: owner 指令「完成自举，编写原生 UI 库，长期实现 GUI 文件格式转换器」——主线仍按门禁先关闭 M22 自举；STEP-0211 用递归下降替换平铺实参管道：实参可为嵌套调用（按名字全模块解析、kind 对 callee 形参逐位检查、内层指令先于外层按子树序发射、result = base + 子树指令数 − 1、区域计数不含自增），与 Rust lower_core byte-identical；累计 29 个正例均真实 build+runner + serde + verifier + Rust lower_core byte differential。下一步定宽运算的字面量实参与 checked 算术（match 块）；之后通用语句块、codegen、自举闭环、原生 UI 与 GUI 转换器。
...
> - last completed active step: STEP-0211 (M22 nested call-argument IR：递归下降实参、内层先发、子树序 result 编号、区域计数，与 Rust lower_core byte-identical；2/2 real-runner suite)；此前 STEP-0210 (fixed-width literal call-argument IR)；STEP-0209 (fixed-width operation IR)。
> - next step: M22 S6 续行——扩展 typed IR：定宽运算的字面量实参、checked 算术（match 块），逐片保持 Rust lower_core canonical JSON byte-identical；随后通用语句块、codegen（RFC-0011）与 ADR-0015 bootstrap harness；自举 GO 后再进入原生 UI/GUI 转换器。
