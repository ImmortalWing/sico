...
> - updated: 2026-09-22 (STEP-0253 general-while no-else frames land byte-exact; canary frontier advances to the list.get match region)
...
> - last completed active step: STEP-0253 (general-while no-else frames; M22 remains NO-GO)；此前 STEP-0252 (nested intrinsic user-call condition)。
> - next step: 为 self-host match lowering 接入 `sico.list.get` result intrinsic 与 ok/error arm payload/project 形状，继续推进 formatter canary；完整 formatter/selfhost bundle 绿后才进入 RFC-0011 general Component codegen 与 ADR-0015 bootstrap harness。
