...
> - updated: 2026-09-22 (STEP-0256 list.get result-match + nested intrinsic returns land byte-exact; canary frontier advances to the line_tokens multi-match region)
...
> - current support step: STEP-0255 完成 M22–M24 计划细化（owner 指令「细化M22-M24文档和step」）：M22 增切片状态表 §3.1 与执行队列 §3.2（`item` list.get match → formatter 尾 17 函数 → selfhost 全源 byte-exact → S5 RFC-0011 扩面 → S6 ADR-0015 自举 → S7 审计），M23 增四项逐项工作协议 §8 与排序依赖图 §9，M24 增逐 gate 工作分解、pilot 循环能力映射表与枚举错误注入语料 §8；不预留未来 STEP 编号、不改任何 gate；M22 计划同时登记验证主机工具链注记（msvc 钉住匹配本机缓存，GNU 路径待 msys2-binutils/gcc 重新供给）。此前 STEP-0254 完成自举差分 harness 的 PreparedProgram 复用：同一 13 项测试串行实测 801.46s → 66.99s，每次输入仍独立 Store。
