...
fn main() {
    ...
    let root = Path::new(arguments.get(1).map_or("tests/end-to-end", String::as_str));
    ...
    for _ in 0..iterations {
        for source in &sources {
            let ir = lower_core(source).expect("M3 corpus must lower");
            artifact_bytes += compile_component(&ir)
                .expect("M3 corpus must compile")
                .len();
        }
    }
    ...
    println!(
        ...
        serde_json::to_string(&json!({
            ...
            "total_builds": sources.len() * iterations,
